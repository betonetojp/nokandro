...
- **bunker://** では、誤ったシークレットを送信した場合は拒否されます。シークレットが空の場合は新規クライアントとして受け入れます。既に認証済みのクライアントはシークレットチェックをスキップして再接続できます。
- **nostrconnect://** では、シークレットの不一致はログに記録されますが接続は常に受け入れられます。

### 接続クライアント管理

- 認証済みクライアントの公開鍵は `HashSet` で管理されます。
- `connect` / `ping` / `get_public_key` 以外のメソッドは、認証済みクライアントからのリクエストのみ受け付けます。
...
- **自動承認**: `connect` でシークレットが検証されれば自動的に承認されます（bunker:// では空シークレットも許可）。リクエストごとの確認ダイアログはありません。
