...
- **bunker://** では、既に認証済みのクライアント（`_authorizedClients` に含まれる）が再接続する場合、シークレットチェックをスキップして接続を許可します（キャッシュされたトークンが古い場合があるため）。新規クライアントの場合は、シークレットが正しく提示されない場合（空または不一致）は接続が拒否されます。
- **nostrconnect://** では、シークレットの不一致はログに記録されますが接続は常に受け入れられます（NIP-46 では、シークレットは認証トークンではなく曖昧さ回避のためのキーと位置付けられています）。

### 接続クライアント管理

- **bunker://** では、認証済みクライアントの公開鍵は `HashSet (_authorizedClients)` で管理され、SharedPreferences に永続化されます。アプリ再起動後も認証済み状態が維持されます。
- 現在接続中のクライアントは別の `HashSet (_connectedClients)` で管理されます。Bunker 再起動時にはクリアされ、クライアント側で再接続が必要です。
- `connect` / `ping` / `get_public_key` 以外のメソッドは、接続中 (`_connectedClients`) のクライアントからのリクエストのみ受け付けます。
- 未接続クライアントからのリクエストは無視されます。
- `get_public_key` は読み取り専用のため、未接続クライアントからでも応答します。
...
- **自動承認**: bunker:// では、既知のクライアント（以前に認証済み）からの接続は自動的に承認されます。新規クライアントの場合、正しいシークレットの提示が必要です。nostrconnect:// では常に接続を受け入れます。リクエストごとの確認ダイアログはありません。
- **bunker:// クライアント管理**: bunker:// では、認証済みクライアントのリストは SharedPreferences に永続化され、アプリ再起動後も維持されます。ただし、Bunker を再起動すると「接続中」状態はクリアされ、クライアント側で再接続が必要です（多くのクライアントは自動再接続を行います）。
