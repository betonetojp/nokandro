...
- **nostrconnect://** では、接続応答の `result` に URI の `secret` を返します。クライアント側で検証が必要です。他アプリから `nostrconnect://` リンクを開くと Bunker タブへ遷移し URI が入力欄に貼り付けられます。
- **bunker://** のシークレットは **Reset Secret** まで固定です（接続のたびにローテーションしません。URI を安定させ、既知クライアントの再接続を容易にします）。

### 接続クライアント管理

- **bunker://** では、認証済みクライアントの公開鍵・表示名・権限文字列は `BunkerClientStore`（SharedPreferences）に永続化されます。アプリ再起動後もペアリング状態が維持されます。
- 接続セッション自体は Bunker 再起動で切れますが、既知クライアントは同じ bunker URI で自動再接続できます。
...
2. 「Delete」を選択すると、保存されたシークレットと **すべてのペアリング情報**（認証済みクライアント）が削除されます。
3. **Bunker が自動的に停止**します。
4. 次回起動時に新しいシークレットで新しい URI が生成されます。

> ⚠️ リセット後は新しい URI の再共有が必要です。既知クライアントの一覧もクリアされます。
...
- **シークレットの一致を確認**
  ...
  - Status に `Secret mismatch` または `invalid secret` エラーが表示される場合、URI が古い可能性があります。Reset Secret → 再起動で新しい URI を生成してください。
...
- **自動承認**: bunker:// では新規クライアントは正しい secret で自動ペアリングされ、既知クライアントはシークレットなしで再接続できます。クライアントを **Remove** しても bunker URI（グローバル secret）は変わりません（**Reset Secret** のみ URI を更新）。NIP-46 の各メソッドごとの確認ダイアログはありません（NIP-55 とは別）。
- **bunker:// クライアント管理**: `BunkerClientStore` に pubkey・名前・perms を保存。Bunker 停止後もペアリングは残り、同じ bunker URI で再接続可能です。
