...
- **Remove** ボタンで切断・削除できます。
- セッションはアプリ再起動後も自動的に復元されます。

### nostrconnect:// の必須パラメータ

NIP-46 に従い、URI には **relay**（1つ以上）と **secret** が必須です。
...
| `ping` | 接続確認 | なし → `pong` |
| `switch_relays` | リレー一覧の取得 | リレー URL 配列または `null` |
...
- **bunker://** では、一度ペアリングしたクライアント（`BunkerClientStore` に保存）が再接続する場合、シークレットチェックをスキップして接続を許可します（Amber と同様の UX）。**新規クライアント**は bunker URI のグローバル secret が一致すれば自動的にペアリングされます。
- **nostrconnect://** では、接続応答の `result` に URI の `secret` を返します。クライアント側で検証が必要です。ディープリンクは URI 貼り付け後に確認ダイアログを経て接続します。
- **bunker://** のグローバル secret は **新規ペア成功時**と **Remove** 時にローテします（**1 URI = 1 新規オンボード**。古い URI の secret で別 pubkey の新規ペアは不可）。画面上の bunker URI とコピーは `ACTION_BUNKER_URI_CHANGED` で即更新。既知クライアントは新 URI なしで再接続可能。**Reset Secret** で全ペア削除＋次回起動時に新 URI。

### 接続クライアント管理

- **bunker://** では、認証済みクライアントの公開鍵・表示名・権限文字列は `BunkerClientStore`（SharedPreferences）に永続化されます。アプリ再起動後もペアリング状態が維持されます。
- 接続セッション自体は Bunker 再起動で切れますが、既知クライアントは secret なしで自動再接続できます（新規ペアリング時のみ現在の bunker URI の secret が必要）。
- `Nip46Handler` が接続済みクライアントのみ NIP-46 メソッドを処理します。
- **nostrconnect://** では、各セッションが個別に管理され、クライアントの認証状態はセッションごとに保持されます。

### 秘密鍵の保護

- 秘密鍵（`nsec`）は Android Keystore および Jetpack Security (`EncryptedSharedPreferences`) を用いて、暗号化された安全な状態で端末内にのみ保存されます。
- 旧バージョンから移行する場合は、初回起動時に自動的かつ透過的に平文から暗号化領域へマイグレーションされます。
- 外部に秘密鍵が送信されることはありません。
- 署名や暗号化/復号の処理は常に nokandro アプリ内で行われ、結果（署名データや暗号文）のみが返却されます。
...
- シークレットは SharedPreferences に保存され、Bunker の再起動時にも同じ URI が維持されます（ペアリングの合間では secret は変わりません）。
- **既知クライアント**の再接続では bunker URI の再入力は不要です。**新規**を追加するたびに secret がローテするため、次の新規クライアントには画面の最新 URI を共有してください（下記「新規ペア成功時のローテ」）。

#### 新規ペア成功時のローテ

**新規クライアント**が初回 `connect` でペアリングに成功すると：

1. `BunkerClientStore` に pubkey が保存されます（既知クライアント化）。
2. 直後に **グローバル secret** がローテし、`pref_bunker_secret` と表示 URI が更新されます。
3. **同じ URI（古い secret）** を別の新規 pubkey が使っても新規ペアできません。次の新規クライアントには画面の **最新 bunker URI** を共有してください。
4. 今ペアしたクライアントは既知のため、secret なしで再接続できます。

#### クライアント削除（Remove）時のローテ

**Remove** で 1 クライアントを削除すると：

1. 当該クライアントのみ `BunkerClientStore` から削除され、接続が切断されます。
2. **グローバル secret** が新規生成され、`pref_bunker_secret` と稼働中 Bunker のメモリ上 secret が更新されます。
3. `ACTION_BUNKER_URI_CHANGED` により、画面上の bunker URI と **Copy** でコピーされる URI が新 secret と一致します。
4. **他の認証済みクライアント**はリストに残り、既知クライアントとして secret なしで再接続できます。削除したクライアントは古い URI では新規接続できません。

#### シークレットのリセット（Reset Secret）

**Reset Secret** ボタンを押すと：

1. 確認ダイアログが表示されます（全ペア削除・Bunker 停止・再起動後の URI 再共有について）。
2. 「Reset」を選択すると、保存されたシークレットと **すべてのペアリング情報**（認証済みクライアント）が削除されます。
3. **Bunker が自動的に停止**します。
4. 次回起動時に新しいシークレットで新しい URI が生成されます。

> ⚠️ リセット後は **すべてのクライアント**に新しい URI の再共有が必要です。
...
- OS がプロセスを殺して Sticky 再起動した場合、Intent extras が無いことがあります。そのときは `pref_bunker_enabled` が ON ならセキュア保存の nsec / リレー / secret から自動復元します。
- 長時間放置後も止まりやすい端末では、Bunker タブの **Exclude from battery optimization** でバッテリー最適化から除外してください（Bunker 有効化時に一度だけ案内ダイアログも出ます）。

### 再接続

- リレーとの接続が切断された場合、`NostrWebSocketClient` が**指数バックオフ自動再接続**を試みます（初期 5秒、失敗するごとに2倍にし最大 60秒まで引き上げ）。接続成功時にディレイは 5秒 にリセットされます。
- WebSocket の `KeepAliveInterval`（30秒）と、受信が長時間（約 10分）止まる場合のフォールバック再接続により、半開き接続の放置を防ぎます。アプリケーションメッセージが無いアイドル状態でも、プロトコル ping で生存確認します。
- 受信バッファには `ArrayPool<byte>` によるメモリレンタル構造を適用しており、頻繁な接続やメッセージ受信におけるヒープ割り当てアロケーションを抑え、GC負荷を大幅に削減しています。
- bunker:// は再接続時も同じシークレット・URI が維持されます。
- nostrconnect:// セッションは SharedPreferences に永続化されており、アプリ再起動時に自動復元・再接続されます。

### 暗号化の自動検出

kind 24133 のペイロードは NIP-44 で暗号化・復号します（NIP-46 準拠）。
...
- **シークレットの一致を確認**
  - 新規クライアントの場合、URI を正確にコピー&ペーストしてください（末尾の空白や改行に注意）。
  - Status に `Secret mismatch` または `invalid secret` エラーが表示される場合、URI（とくに `secret=`）が古い可能性があります。Bunker タブの **Copy bunker URI** で表示中の最新 URI を使ってください（新規ペア成功後・**Remove** 後に secret は変わります）。
  - すべてのペアをやり直す場合のみ **Reset Secret** → Bunker 再起動で新しい URI を配布してください。
  - 既に認証済みのクライアントが再接続する場合、シークレットチェックは省略されます。
...
- **リレーの到達性を確認**
  ...
  - 実装上、URI にリレー情報が含まれていない場合はパースエラーとなり接続できません（NIP-46に従い、少なくとも1つ以上のリレー指定が必須です）。
...
- **自動承認**: bunker:// では新規クライアントは現在の secret で自動ペアリングされ、成功直後に secret をローテ（1 URI = 1 新規）。既知クライアントはシークレットなしで再接続。**Remove** でもローテ。**Reset Secret** で全ペア削除＋次回起動で新 URI。NIP-46 の各メソッドごとの確認ダイアログはありません（NIP-55 とは別）。
- **bunker:// クライアント管理**: `BunkerClientStore` に pubkey・名前・perms のみ保存（per-client secret は保持しない）。Bunker 停止後もペアリングは残り、既知クライアントは secret なしで再接続可能です。
- **nostrconnect:// セッション永続化**: nostrconnect:// セッションは SharedPreferences に保存され、アプリ再起動後も自動復元・再接続されます。クライアントリストからの手動削除は即座に反映されます。
