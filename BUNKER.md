...
- **bunker://** では、一度ペアリングしたクライアント（`BunkerClientStore` に保存）が再接続する場合、シークレットチェックをスキップして接続を許可します（Amber と同様の UX）。新規クライアントは正しいシークレットの提示が必要です。
- **手動承認**: 「Approve new bunker clients manually」を ON にすると、新規クライアントは pending 一覧に載り、承認するまで `connect` は拒否されます。
- **nostrconnect://** では、接続応答の `result` に URI の `secret` を返します。クライアント側で検証が必要です。
...
2. 「Delete」を選択すると、保存されたシークレットと **すべてのペアリング情報**（認証済み・pending クライアント）が削除されます。
...
- **自動承認**: bunker:// では既知クライアントはシークレットなしで再接続できます。手動承認モード ON 時は新規のみ UI で承認が必要です。NIP-46 の各メソッド（`sign_event` 等）ごとの確認ダイアログはありません（NIP-55 とは別）。
